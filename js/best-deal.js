...
document.addEventListener('DOMContentLoaded', () => { // Структура сторінки загружена і готова до взаємодії
   ...
   // об'єкт з назвами місяців для виведення в html-розмітку
    ...
    // lite
    dateLite.textContent = `${liteDate.getUTCDate()} ${MONTHS[liteDate.getUTCMonth()]} ${liteDate.getUTCFullYear()}`;

    // basic
    dateBasic.textContent = `${basicDate.getUTCDate()} ${MONTHS[basicDate.getUTCMonth()]} ${basicDate.getUTCFullYear()}`;

    // pro
    datePro.textContent = `${proDate.getUTCDate()} ${MONTHS[proDate.getUTCMonth()]} ${proDate.getUTCFullYear()}`;
})